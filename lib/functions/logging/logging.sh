...
# FIXME-REFACTOR(Krey): This whole file needs spec and rewrite
...
logging_init() {
	...
	# if on Mac, add a an apple emoji to the left marker wrapped in dark gray brackets
		...
		[ "$wsl2_type" = "none" ] || {
			...
			left_marker="${gray_color}[${windows_emoji}|${normal_color}"
		}
	fi
}
...
if_user_on_terminal_and_not_logging_add() {
	...
	[ -z "$CURRENT_LOGGING_SECTION" ] || return 0

	# REFACTOR(Krey): I gave up refactoring here, this whole file needs rewrite..
