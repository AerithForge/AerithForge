###! # Initiate
###! For processing of CLI arguments and declaration of variables for further processing

# FIXME-REFACTOR(Krey): Messy mess, needs to be footprinted, documented, spected and refactored

#? Function designed to be called from the entry-file to the Framework and used as an entrypoint to the framework which processes arguments into workable variables for further processing.
#?
#? Sets the following variables:
#? * `ARMBIAN_ORIGINAL_ARGV` - CLI arguments parsed in the framework prior to processing
#?
#? ...
framework_initiate() {
	ARMBIAN_ORIGINAL_ARGV=("${@}") #? Stores the CLI arguments parsed in the framework
	#(new) FRAMEWORK_ORIGINAL_ARGV="$@" #? Stores the CLI arguments parsed in the framework

	# WTF(Krey): What's ARMBIAN_ENABLE_CALL_TRACING?
	if [ "$ARMBIAN_ENABLE_CALL_TRACING" = "yes" ]; then
		[ -d "$SRC/output/call-traces" ] || mkdir -p "$SRC/output/call-traces"
		printf "%s\n" "" > "$SRC/output/call-traces/calls.txt"
	...
	declare -g -A ARMBIAN_PARSED_CMDLINE_PARAMS=() # A dict of PARAM=VALUE
	declare -g -a ARMBIAN_NON_PARAM_ARGS=() # An array of all non-param arguments
	...
	ARMBIAN_CONFIG_FILES=() # fully validated, complete paths to config files.

	# only valid command and handler will ever be set here.
	unset ARMBIAN_COMMAND_HANDLER ARMBIAN_COMMAND ARMBIAN_COMMAND_VARS

	# FIXME-QA(Krey): This should fail with error about unknown parameter
	ARMBIAN_HAS_UNKNOWN_ARG="no" # if any unknown params, bomb.
	...
	# More sanity checks.
	# If unknowns, bail.
	[ "$ARMBIAN_HAS_UNKNOWN_ARG" != "yes" ] || exit_with_error "Unknown arguments found. Please check the output above and fix them."
	...
	ARMBIAN_CHANGE_COMMAND_TO="$ARMBIAN_COMMAND"
	while [ -n "$ARMBIAN_CHANGE_COMMAND_TO" ]; do
		display_alert "Still a command to pre-run, this time:" "$ARMBIAN_CHANGE_COMMAND_TO" "debug"

		ARMBIAN_COMMAND_REQUIRE_BASIC_DEPS="no" # reset this before every pre_run, so only the last one wins.
	...
	# Init basic dirs.
	## DEST is the main output dir, and USERPATCHES_PATH is the userpatches dir. read-only.
	DEST="$SRC/output"
	USERPATCHES_PATH="$SRC/userpatches"
	...
	if [ -n "$ARMBIAN_BUILD_UUID" ]; then
		...
		if command -v uuidgen 1> /dev/null; then
			...
			# shellcheck disable=SC2004 # $/${} is unnecessary on arithmetic variables, but makes this mess more readable
	...
	WORKDIR_BASE_TMP="$SRC/.tmp" # a.k.a. ".tmp" dir. it is a shared base dir for all builds, but each build gets its own WORKDIR/TMPDIR.

	WORKDIR="$WORKDIR_BASE_TMP/work-$ARMBIAN_BUILD_UUID" # WORKDIR at this stage. It will become TMPDIR later. It has special significance to `mktemp` and others!
	LOGDIR="$WORKDIR_BASE_TMP/logs-$ARMBIAN_BUILD_UUID" # Will be initialized very soon, literally, below.
	EXTENSION_MANAGER_TMP_DIR="$WORKDIR_BASE_TMP/extensions-$ARMBIAN_BUILD_UUID" # EXTENSION_MANAGER_TMP_DIR used to store extension-composed functions

	# @TODO: These are used only by rootfs/image actual build, move there...
	SDCARD="$WORKDIR_BASE_TMP/rootfs-$ARMBIAN_BUILD_UUID" # SDCARD (which is NOT an sdcard, but will be, maybe, one day) is where we work the rootfs before final imaging. "rootfs" stage.
	MOUNT="$WORKDIR_BASE_TMP/mount-$ARMBIAN_BUILD_UUID" # MOUNT ("mounted on the loop") is the mounted root on final image (via loop). "image" stage
	DESTIMG="$WORKDIR_BASE_TMP/image-$ARMBIAN_BUILD_UUID" # DESTIMG is where the backing image (raw, huge, sparse file) is kept (not the final destination)
	...
	ARMBIAN_LOG_CLI_ID="${ARMBIAN_LOG_CLI_ID:-$ARMBIAN_COMMAND}"
