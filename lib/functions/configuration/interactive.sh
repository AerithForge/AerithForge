###! # Interactive
###! For functions that handle interactive user input through Terminal User Interface ("TUI")

interactive_config_prepare_terminal() {
	# WTF(Krey): What is ROOT_FS_CREATE_ONLY?
	if [ -z "$ROOT_FS_CREATE_ONLY" ]; then
		if [ -t 0 ]; then # "-t fd return True if file descriptor fd is open and refers to a terminal". 0 = stdin, 1 = stdout, 2 = stderr, 3+ custom
			...
			[ -z "$COLUMNS" ] || stty cols "$COLUMNS"
			[ -z "$LINES" ] || stty rows "$LINES"
			TTY_X="$(($(stty size | awk '{print $2}') - 6))" # determine terminal width
			TTY_Y="$(($(stty size | awk '{print $1}') - 6))" # determine terminal height
		fi
	fi
	# We'll use this title on all menus
	#backtitle="Armbian building script, https://www.armbian.com | https://docs.armbian.com | (c) 2013-2023 Igor Pecovnik "
	backtitle="$FRAMEWORK_NAME Interactive Terminal User Environment, $FRAMEWORK_WEBSITE | $FRAMEWORK_WEBSITE_DOCS"
	ARMBIAN_INTERACTIVE_CONFIGS=() # An associative array of all interactive configurations
}

# DNR-SECURITY(Krey): Eval is Evil!
# Set config variable and ARMBIAN_INTERACTIVE_CONFIGS in a consistent way
# $1: variable name
# $2: variable value
set_interactive_config_value() {
	eval "$1"='$2'
...
# WTF(Krey): What is this supposed to be? Legacy thing weirdness? Shorthand?
interactive_config_ask_kernel() {
	interactive_config_ask_kernel_configure
}

interactive_config_ask_kernel_configure() {
	# The option 'FRAMEWORK_KERNEL_CONFIGURE' is used to skip kernel configuration
	[ "$FRAMEWORK_KERNEL_CONFIGURE" = "yes" ] || return 0
...
get_list_of_all_buildable_boards() {
	...
	[ -z "$2" ] || local -n ref_arr_all_board_options="$2" # optional
	local -n ref_dict_all_board_types="$3"
	local -n ref_dict_all_board_source_files="${4}"
	[ -z "$5" ] || local -n ref_dict_all_board_descriptions="${5}" # optional

	declare -i prepare_options=0
	if [[ -n "$2" || -n "$5" ]]; then # only if second or fifth reference specified, otherwise too costly
		prepare_options=1
	fi
...
interactive_config_ask_board_list() {
	...
	# FIXME-QA(Krey): This should be made to store boolean for processing alike:
	## if [ ! "$EXPERT" ]; then
	## 	WIP_STATE=supported
	## else
	...
	case "$EXPERT" in
		...
		"yes")
			WIP_STATE=supported
			;;
		*) exit_with_error "The variable 'EXPERT' has been set to an unrecognized variable"
	esac

	WIP_BUTTON='CSC/WIP/EOS/TVB'
	STATE_DESCRIPTION=' - boards with high level of software maturity'

	# FIXME(Krey): This needs to be handled
	# @TODO: this is a _very_ early call to mktemp - no TMPDIR set yet - it needs to be cleaned-up somehow
	declare temp_rc
	temp_rc="$(mktemp)"

	# FIXME-QA(Krey): Potential for an infinite loop
	while true; do
		declare -a arr_all_board_names=() arr_all_board_options=()                                                                                              # arrays
		declare -A dict_all_board_types=() dict_all_board_source_files=() dict_all_board_descriptions=()                                                        # dictionaries
		get_list_of_all_buildable_boards arr_all_board_names arr_all_board_options dict_all_board_types dict_all_board_source_files dict_all_board_descriptions # invoke
...
interactive_config_ask_branch() {
	# FIXME(Krey): Replace with SOMETHING_BRANCH?
	...
	[ -n "$BRANCH" ] || exit_with_error "No kernel branch selected"
	return 0
}

interactive_config_ask_release() {
	[ -z "$RELEASE" ] || {
		display_alert "Already set RELEASE, skipping interactive" "$RELEASE" "debug"
		return 0
	}
	...
	[ -n "$RELEASE" ] || exit_with_error "No release selected"
...
interactive_config_ask_standard_or_minimal() {
	...
	[ -n "$BUILD_MINIMAL" ] || exit_with_error "No standard/minimal selected"
